...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("trade_processing.log")],
)
...
def prepare_order_params(trade: dict, broker: Broker) -> dict:
    ...
    order_params = {
        "type": trade.get("type"),
        "order": {"pair": pair, "quantity": quantity, "price": price, "side": side},
    }
...
async def process_orders_async(brokers: List[Broker], trade: dict) -> Dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=min(32, len(brokers))) as executor:
        for broker in brokers:
            ...
            task = loop.run_in_executor(
                executor, lambda b=broker, op=order_params: send_order_async(b, op)
            )
            tasks.append(task)
    ...
    logger.info(
        f"Processed {len(brokers)} orders in {elapsed_time:.2f} seconds. Success: {success_count}, Errors: {error_count}"
    )
...
def process_trade(trade: dict):
    ...
    if trade_type == "entry":
        ...
        logger.info(
            f"Processing orders for {len(users_broker)} users with strategy {trade_strategy}"
        )
        ...
        # Log the results
        logger.info(
            f"Order processing complete. Processed {results['total_orders']} orders in {results['elapsed_time']:.2f} seconds."
        )
        logger.info(
        ...
        results = loop.run_until_complete(process_orders_async(users_broker, trade))
        logger.info(
            f"Stop loss order processing complete. Processed {results['total_orders']} orders in {results['elapsed_time']:.2f} seconds."
        )
        ...
        results = loop.run_until_complete(process_orders_async(users_broker, trade))
        logger.info(
            f"Exit order processing complete. Processed {results['total_orders']} orders in {results['elapsed_time']:.2f} seconds."
        )
